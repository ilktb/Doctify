    ...
    <body>
        ...
        <form action="" method="POST">
            <?php
                ...
                $arr=$db->query("SELECT * FROM applications WHERE trace='1' AND id='$id'")->fetch();            
                $type = $arr['type'];
                $name = $arr['name'];
                $faculty = $arr['faculty'];
                ...
                echo "  <p id='title'> $type </p>
                        ...
                        {
                            $db->prepare("UPDATE `applications` SET `trace`= 2 WHERE id=$id")->execute();
                            header("location: DSHome.php");
                        } 
