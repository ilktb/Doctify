    ...
    <div id="home">
        ...
        <form action="" method="POST">
            <?php
                ...
                $arr=$db->query("SELECT * FROM applications WHERE trace='3' AND id='$id'")->fetch();            
                $type = $arr['type'];
                $name = $arr['name'];
                $faculty = $arr['faculty'];
                ...
                echo "  <p id='title'> $type </p>
                        ...
                        {
                            $db->prepare("UPDATE `applications` SET `trace`= 4 WHERE id=$id")->execute();
                            header("location: DeanHome.php");
                        } 
                        if(isset($_POST["decline"]))
                        {
                            $db->prepare("UPDATE `applications` SET `trace`= 6 WHERE id=$id")->execute();
                            header("location: DeanHome.php");
                        }
